...
**マップ2（ホワイトパス／corridor峡谷）で判明:**

6. **プレビューカメラの画面右はワールドの -X**（standAngleの基底が鏡像）。左右を取り違えると
   「作ったのに写らない」が起きる
7. **corridorの峰は間隔が半径と同程度になると裾が融合する**。融合は連続した稜線を作るので
   むしろ望ましいが、**行軸±60°以内の方位では `surfaceHeightAt` が隣の峰の高さを返す**。
   斜面に物を置くときこれで数百m先に埋没する
8. **斜面のノードは「等高さ間隔」で取るな、「半径等間隔」で取れ**。等高さで取ると1ステップあたりの
   水平距離が32m〜216mと暴れ、つづら折りの折返し間隔が滅茶苦茶になる
9. **ヘアピンで接線を線形補間すると1頂点で向きが180°反転**し、リボンの法線が破綻して
   ダート状のゴミが出る。smoothstepでイージングしろ
10. **峰の変位スパーで斜面が全カメラから遮蔽されることがある**（描画されているのに見えない）。
    位置特定には**対象を一時的にマゼンタ単色にする**のが最速。稜線側の行程は裾の0.58倍までに抑える
11. **道を「面全体を明るくする」で作るとデカール／落書きに見える**。正解は
    **暗い路面＋明るい細い路肩の縁＋落ち着いた法面**の3層。遠景で道として読ませるのは
    明度の高さではなく、暗い帯を明るい線が挟むコントラスト
12. **斜面に立つ構造物（坑門・擁壁）は「食い込ませ＋段状の翼壁」**。単一の長い箱は
    緩斜面で端が70m浮く。段ごとに `surfaceHeightAt` で接地させろ
13. `env.surfaceHeightAt` は**build時に数千回呼ぶと数百msかかる**（マップ2は約7800回）。
    マップ切替時に一度だけ乗るコストなので許容範囲だが、実機fpsは別途要確認
