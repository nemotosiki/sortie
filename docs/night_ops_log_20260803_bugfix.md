...
### Batch 10 — tools群(01:2x適用、並走班の常用経路なので後方互換を最優先)
- [S2] inline_payloadの重複判定が`includes`の部分一致→**前方一致する名前が永久に入らない**
  (`ground_tel`が既にあると`ground_telemetry`が拒否される)→行末一致へ。
  実測: 既存`aircraft_e2d`がある状態で`aircraft_e2`が正しく通ることを確認、重複拒否も維持
- [S2] BEGIN/ENDマーカーを`indexOf`の最初の出現で探しており、payloadが仕様書を
  コメント引用するだけで挿入点が他人の登録の中へ移動しうる(しかも構文は通る)→
  **行完全一致で各1件であることを検査**して失敗させる
- [S3] `registry_gate --update`が損失を件数しか出さずに正本化(inline_payloadが
  最後に案内するコマンド=マージ直後に習慣で叩かれる)→**受理する前に消失一覧を出す**

### Batch 11 — 掃引判定と装飾配置(01:4x適用)
- [S2] ミサイルの地形判定が移動後の終点1点だけ→巡航で1フレーム約14m進むため、
  それより細い尾根を掛け抜けた(コメントが「地中を飛び続けるのを防ぐ」と契約を明記して
  いるのに満たしていない)。→**中点も掃引サンプル**。高度400m超では地形が届かないので
  スキップ=常用時の追加レイキャストは0
- [S2] sea stack(rocks)の配置リングが900〜3300m固定で、desertBasin(keepClear r=3400)と
  stormOcean(r=2400)では大半が`blocked`で無言破棄。砂漠は進入路側が全滅し背後にしか
  残っていなかった→**keepClear円の外側から始まるようリング内径を自動算出**
- **見送り(所見を検証して棄却)**: CIWSのサブステップ非同期。所見は「低fpsで迎撃率が下がる」
  だったが、確率は`1-(1-p)^dt`でdt補正済みで**交戦帯800→320mを通過する総確率はfps非依存**。
  実害なしと判断し、意図的な320m内側の穴(設計)も維持

### 最終検証(01:1x)
**全40ミッション: 正しいキーで起動 40/40・pageerror 0**(米編20/露編20)。
露編は今夜のBatch7修正まで1本も起動できていなかったので、全数通過は今夜が初。

### Batch 12 — 第2ラウンド精読の確定分(05:2x)
第2ラウンドは**8レンズの横断的な問い**（状態遷移マトリクス/境界値/跨ぎ契約/dt vs rawDt/
生成と解放の対/敵AIの縮退/セーブ互換/HUDと内部値のズレ）で実施。裏取り済みを適用:
- [S2] **勝利ホールド中に敵弾で負ける**: MISSION ACCOMPLISHEDの2.8秒間もAA/CIWS/機銃が
  当たり、勝ちがFAILEDに化けて記録も残らなかった（戦域判定と飛行中ミサイルは既に無効化
  されていて、新規攻撃だけ穴）。→`applyPlayerDamage`の非bypass経路にoutcomePendingを追加。
  **自機のクラッシュはbypass経路なので従来通り負ける**（設計意図の通り）
- [S2] **敵ミサイルの信管が掃引されていなかった**: ブースト中の正面接近は接近速度~1378m/s＝
  1フレーム23mに対し信管13m。命中コースの弾がサンプル点の間をすり抜けて飛び去っていた
  （自機弾は既に掃引済みで、敵側だけ修正前の形）。フレアの15m判定も同じ穴。
  →既存の`sweptMissDistance`で移動前に線分判定
- [S2] `updateFlightEffects`が実時間駆動: キルカムのスローモ1.6秒でGストレインが1.2溜まり
  （実際の引き起こしは0.29秒分）、風ストリークが止まった世界を全速で流れていた→シム時間へ
- [S3] 失速警告が飛行終了後も点灯（`stalling`が凍結したまま）→飛行中ゲートを両半分に
- [S3] ウェーブクリア予約のCHECKPOINTバナーが、1.4秒以内に撃墜されるとMISSION FAILEDの
  上に成功トーンで出る→ミッション終了時にキャンセル
- **A/B検証**: HEAD版(8096)と作業版(8094)で勝利ホールド→missionComplete遷移を計測、
  **両方7.9秒で一致=退行なし**。勝利ホールド中に体力40を維持（敵弾が通らない）ことも実測
- 見送り: story payloadの`PRIORITY_CRITICAL=2`（正本は3。署名台詞が黙って捨てられる）は
  **payload領域＝バッチ班の所有物**なので報告のみ。機銃の対艦許容半径(空母で実効350m)は
  バランス隣接のためユーザー判断へ

### Batch 14 — 境界値/フレームレート依存/契約違反(挙動を変えない堅牢化)
- [S2] 海面法線タイルの添字`[1]`/`[2]`が無防備（ガードは**別配列(offsets)の長さ**を見ていた）。
  payloadプリセットが`normalMultipliers`を2要素で渡すと`camX/undefined`=NaNが生きた
  シェーダユニフォームへ入り、そのマップの海が以後ずっと破綻→`[0]`と同じフォールバックへ
- [S2] `clamp(v, 0.02, maxHitChance)`は**上限が下限を下回ると下限が勝つ**ので、
  `maxHitChance:0`（非武装宣言）のマウントが2%命中してしまう。現状はrange/tracer側の
  二重ガードで到達していないだけ→0なら0で確定させる
- [S3] 爆弾の姿勢追従だけ`damping()`を通しておらずフレームレート依存だった→damping化
- [S3] 脅威表示が毎フレーム`Vector3`をnew（更新ループに残った唯一の確保）→スクラッチ再利用
- [S3] カメラup合成の零ベクトルガード（rollFollow 0.5ちょうど＝CLOSEプロファイル）

### 第4ラウンド精読（実プレイ体験の6レンズ）→ Batch25〜27
初見体験／長時間セッション／失敗経路／難易度／解禁と記録／演出整合。所見の全文は
`docs/findings_bughunt_r4_20260803.md`。

- **Batch25 `a71c459`**: 敵機の地形クランプが**移動前の床**を使っていた。833m/sで斜面へ
  向かうと1フレーム41mの間に上がった錐面を無視して沈み、次フレームで跳ね返る＝
  「クランプはメートル単位でしか動かさない」という契約が高速時に破れていた。
  **A/B実測(m-glacier、1200サンプル): 最悪クリアランス 29.76m→38.57m、床割れは両者0**
- **Batch26 `129cead`**: 操作凡例に**旋回の仕方が書いていなかった**（機銃・兵装・ヨー・
  加減速はあるのにピッチ/ロール/目標切替が無い＝初見は曲がり方を知らずに出撃する）／
  難易度の◀▶が**クリックできず**変更手段がキーボード←→のみだった（他のメニューは全て
  クリック可なので、マウスで進んだ人はここで手が止まる）。**実測: クリックでNORMAL→HARD→NORMAL**
- **Batch27 `19b5a1b`**: **基地/市街への被害がランクにも記録にも効いていなかった**。
  ブリーフィングが「基地の被害は君の評価に響く」と約束し、コードのコメントも
  "costs the player a rank" と書いているのに、表示用スコアしか減っていなかった。
  **A/B実測: ペナルティ2000を出してクリア → 修正前 TOTAL 4550(無傷と同一) / 修正後 2550**。
  被害ゼロのクリアは両者4550で不変

**★並行編集の巻き込み(2件目・記録)**: `19b5a1b` には**モデルバッチ班が作業中だった
HUD改装の削除232行**（`#targetDirectionArrow` 系CSSの撤去）が同梱されている。
`git commit --only <path>` はファイル単位なのでハンク分離にならない（1件目 `109ced4` と同型）。
**破壊ではない**ことは確認済み: 残骸参照ゼロ（相手の作業が一貫した状態だった）、
registry_gate緑、実機で起動〜交戦〜カメラ/目標切替までpageerror 0。

### 第3ラウンド精読（未踏の8切り口）→ Batch18〜24
性能ホットパス／AI縮退／音響／入力エッジ／文言／物理積分／描画順／ミッション進行の8レンズ。
所見の全文は `docs/findings_bughunt_r3_20260803.md`。確定分を5バッチで出荷:

- **Batch18 `a835057`**: 艦の操舵方位2箇所がX軸鏡像（ファイル自身が`placeOnRoute`で
  「方向(Dx,Dz)を向く heading=atan2(-Dx,-Dz)、他方は鏡像」と明記していたのに違反）。
  海域フェンスは原点の東西にいる艦ほど外向きに舵を切り、山回避は島の東西にいる艦を島へ突進させていた。
  **A/B実測: フェンス通過後120秒の原点距離が 1918→2303 単調増加(修正前) / 1902→2008でピーク→1583へ復帰(修正後)**
- **Batch19 `57a1f89`**: 友軍空母にも同じ鏡像（護衛対象が盤外へ）／沈没中の艦のマウントが
  死亡座標に取り残されて水面上で燃え続ける（コメントは「alive or wrecked で追従」と宣言）／
  護衛目標の無いミッションで「CONVOY CLEAR · 1/0 SAVED」が出うる
- **Batch20 `cc7a1c8`**: ミッション表の不備で**クリア不能＋バナー無限ループ**（Escも効かない）に
  なる経路を、勝ちとして畳みつつ原因をconsole.warnで名指しする形に。spawnShipだけ未登録艦種の
  検査が無く、fleetのtypo一つで毎フレーム例外＝実質フリーズになりえた
- **Batch21-24 `0aa27ae`**: コックピット視点で損傷煙が出ない（可視フラグを生存フラグの代用に
  していた）／星が不透明物の手前／**メニューのキーリピートが飛行入力へ漏れて出撃直後に機体が
  勝手にロール**（実測: KeyD押しっぱなしで出撃してもrollRate 0）／パッド長押しラッチの取り残し／
  全艦種が「イージス艦、沈黙」と呼ばれる／市街爆撃で「基地被弾」／タブを離れると
  エンジン音とロック音が鳴り続ける／エンジン停止のクリック

**見送り**: 性能最適化（syncGameHookの毎フレームO(M²)等）。SwiftShaderのヘッドレスでは
GPU律速でフレーム213msとなり**改善を測れない**ため、盲目的な最適化を避けた。所見はr3に保存。

### Batch 16 — 爆弾の着弾面を設置物の立つ面へ揃える（自分の変更の波及漏れ）
Batch5で設置物・友軍基地・投光器を無クランプ面(`surfaceTopAt`)へ移したが、**爆弾側5箇所は
クランプ版のまま**だった。夜間基地の平頂(92m)に対し爆弾は84.64mを地面と見なすため、
**CCIPの照準点が目標の立つ甲板より7.36m下**に出て、落下しきる分だけ着弾も長く流れていた。
CCIP2箇所・爆弾の地表判定2箇所・爆風の基準面1箇所を`surfaceTopAt`へ統一。
**実測: CCIP予測 y=84.64 → 92.00（目標の甲板と一致）**。
検出経緯: 「UGBが本当に動くか」を実機で確かめる過程で気づいた＝**自分の修正の後始末**。

### ★調査の訂正記録（自分の誤りを潰した経緯）
「CLOSEカメラ+背面飛行でカメラ姿勢が永久NaN」という**重大バグを検出したと思い、
根治パッチ(Batch15)まで書いて適用した**。が、A/Bで修正前後とも同じNaNが出たため
「既存バグ」と判断→さらに調べると**プローブ側の誤用**（`forceAttitude`は位置引数
`(yaw, pitch, bank)`なのにオブジェクトを渡していた＝degToRad(object)でNaN）が原因と判明。
正しい呼び方では**両ビルドともNaNなし**。→**Batch15は全面撤回**し、Batch14に残る
カメラ用ガードのコメントも「観測された症状」ではなく「算術上の退化ガード」と書き直した。
**再現しない症状を根拠としてコードに書き残さない**ため。

### 追加指示: キル時ポップアップの全廃(04:3x、`109ced4`)
ユーザーから「キル時『+1000 · Tu-95』も消して」。表示だけ止めると死にコードが残るので
**ポップアップ機構ごと除去**(CSS3ブロック/DOMコンテナ/定数/ui参照/配列/3関数/毎フレーム更新/
リセット/debugフック)。実機で3機撃墜→ポップアップ0・コンテナ非存在・スコアとHUDは正常・エラー0。

**★並行編集の巻き込み(記録)**: このコミットには**モデルバッチ班の未コミットだったコックピットHUD
CSS(31行)が同梱**されている。`git commit --only <path>` は「その1ファイルだけ」を確定するが、
**そのファイルの作業ツリー全体**を確定するのでハンク単位の分離にはならない。破壊はしていない
(相手の成果が私のメッセージで先に確定しただけ)。ハンク分離が要るときは
`git diff`→自分のマーカーでフィルタ→`git apply --cached`→`git commit`(作業ツリー無接触)が正解。

### 長時間実戦ストレステスト(02:1x、m-carrier)
6サイクル×(4機増援スポーン→ミサイル/SP.W連射→カメラ切替→6機撃墜→補給)+リトライ5連打。
| 指標 | baseline | final | 判定 |
|---|---|---|---|
| DOMノード数 | 706 | 706 | **増加0**(HUD固定プール契約が保たれている) |
| 音源数 | 0 | 0 | **増加0** |
| パーティクル | 4 | 156(減衰中) | 交戦中773→無交戦で復帰=プール契約OK |
| geometries | 92 | 100 | cycle3で頭打ち・以降のサイクルとリトライで不動=**リークなし**(盤面の生存機分) |
