...
- **Su-37**: AC7では4AAMを積む。しかし本書では**依頼どおりQAAMを採用する**。理由:
  ...
  （この重複はAC7側にも実在する）。
  「推力偏向で敵の背後に貼り付く機体」には、全ミサイル共通上限100°/sの通常MSLと同じ旋回率でも、実際のオーバーシュート後に**一度だけ再捕捉して長く追えるQAAM**のほうが
...
- `turnRateDeg: 55`: QAAM/通常MSL（100°/s上限）より**大幅に低い**。動かない・鈍い目標にしか当たらない鈍重な弾。
...
**設計根拠**:
- `multi: 4`: ロックコーン内の最大4目標が各自の0.85秒を**並行して**進める。最初から圏内にいる4目標は0.85秒後に同時ロックし、1目標ずつ0.85秒を4回待つキューにはしない
...
**共通実装**:
`updateMultiLock()` は兵装名をハードコードせず、`weapon.multi` と候補IDを共通の並行ロック状態機械へ渡す:
```js
updateConcurrentMultiLock(multiLock, dt, candidateIds, weapon.multi, LOCK_TIME, MULTI_LOCK_GRACE);
```
`launchSpSalvo()` への分岐も `if (PLAYER_SPW.multi > 1)`（L6119）という汎用条件。
→ 4AAM／6AAM／8AAM／4AGMは同じ「上限数まで並行ロック→ロック済み数を一斉発射」を共有する。

**干渉なし条件**:
- `multi: 1` の既存3種は `updateMultiLock` が即 `clearMultiLock()` するため**影響なし**
- 8AAM(F-22)も同じ並行ロック契約へ統一し、違いは上限数・弾頭・旋回性能・装填数だけとする
...
4. **4AAM は共通の並行ロック状態機械を使用**し、最大4目標を0.85秒で同時ロックする（§3-2）
