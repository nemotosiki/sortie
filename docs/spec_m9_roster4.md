...
**設計根拠**:
- `multi: 6` / `multiLockTime: 0.45`: 8AAM/4AAM と同じ機構。**6目標で0.45秒刻み＝満載まで2.7秒**
  （8AAM 3.6秒 / 4AAM 1.8秒 の**ちょうど中間**）。3種が綺麗な階段になる
...
**★実装コストはゼロ（fighters.md §3-2 と同じ根拠）**:
`updateMultiLock()`（L6782-6797相当）は `weapon.multi` と `weapon.multiLockTime` を読む**汎用実装**で、
`launchSpSalvo()` への分岐も `if (PLAYER_SPW.multi > 1)` という汎用条件。
→ **6AAM は `SPW_TYPES` にデータを1件足すだけで動く。新規コードは不要。**
...
- 8AAM(F-22) / 4AAM(F-35C) とはデータが独立しており、両機の挙動は変わらない
