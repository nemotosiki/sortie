...
def restore_index() -> None:
    ...
    # Restore the on-screen contact frame exactly to the code immediately before
    # 087e50ed ("Apply red white blue contact colours"). Internal disposition
    # metadata remains; only the HUD rendering goes back.
    source = replace_once(
        source,
        "      border: 1px solid currentColor;\n      color: #f4f7fa;",
        "      border: 1px solid rgba(112, 255, 151, 0.92);\n      color: #78ff9d;",
        "legacy enemy marker border/color",
    )
    source = replace_once(
        source,
        "      filter: drop-shadow(0 0 5px rgba(244, 247, 250, 0.55));",
        "      filter: drop-shadow(0 0 5px rgba(65, 255, 126, 0.68));",
        "legacy enemy marker glow",
    )
...
// Gameplay classification stays intact for Sera mission logic.
...
// HUD rendering is deliberately the historical pre-three-colour implementation
// from ae212b6983d1c48de9a0e4fadfc714c11998075e.
