...
const CHECK = path.join(ROOT, "tools/check_campaign_economy_e2e.mjs");
...
if (!fs.existsSync(CHECK) || fs.readFileSync(CHECK, "utf8") !== checkSource) {
  fs.writeFileSync(CHECK, checkSource);
  fs.chmodSync(CHECK, 0o755);
}
fs.writeFileSync(MESSAGE, "Add campaign isolation browser regression gate\n");
...
run(["--check", "tools/check_campaign_economy_e2e.mjs"]);
run([
...
console.log("apply_campaign_isolation_phase: browser regression gate prepared");
