...
async function openMission() {
  const opened = await openPage(`${baseUrl}/index.html`);
  ...
  for (const payload of ["map_karanPlain", "mission_sera_m08", "mission_sera_m09"]) {
    assert(payloads.includes(payload), `integrated campaign did not apply ${payload}`, payloads);
  }
...
try {
  ...
  const map = await openPage(
    `${baseUrl}/index.html?worldPreview=karanPlain`
  );
  ...
  const mapSurface = await openPage(
    `${baseUrl}/index.html?worldPreview=karanPlain&worldPreviewDetail=surfaceQa`
  );
