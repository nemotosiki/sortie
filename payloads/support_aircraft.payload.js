...
export default function register(ctx) {
  ...
  // EYE OF THE STORM: add one real friendly maritime-patrol aircraft. The
  // deployment API queues this object until FRIENDLY_DEPLOYMENTS exists. A
  // getter copies the proven m-escort air-guard contract from the public debug
  // snapshot at that exact moment, without reaching into the private table.
  function liveAirGuardTemplate() {
    const snapshot = typeof window !== "undefined"
      && window.__game?.debug?.friendlyDeployments?.();
    return snapshot?.["m-escort"]?.guard || null;
  }

  function patrolGuardFrom(template) {
    ...
    // already present on the proven template are changed.
    const setIfPresent = (key, value) => {
      if (Object.prototype.hasOwnProperty.call(guard, key)) guard[key] = value;
    };
    const pointLike = (current, point) => Array.isArray(current)
      ? [point.x, point.y, point.z]
      : { ...point };
    const setPointIfPresent = (key, point) => {
      if (!Object.prototype.hasOwnProperty.call(guard, key)) return;
      guard[key] = pointLike(guard[key], point);
    };
    setIfPresent("kind", "transport");
    setIfPresent("type", "maritimePatrol");
    setIfPresent("aircraft", "maritimePatrol");
    setIfPresent("aircraftType", "maritimePatrol");
    setIfPresent("label", "PATROL 1");
    setIfPresent("callsign", "PATROL 1");
    setIfPresent("count", 1);
    setIfPresent("hp", AIRCRAFT_TYPES.maritimePatrol.maxHealth);
    setIfPresent("maxHp", AIRCRAFT_TYPES.maritimePatrol.maxHealth);
    setIfPresent("speed", 138);
    setIfPresent("spacing", 0);
    setPointIfPresent("start", start);
    setPointIfPresent("spawn", start);
    setPointIfPresent("entry", start);
    setPointIfPresent("from", start);
    setPointIfPresent("exit", exit);
    setPointIfPresent("end", exit);
    setPointIfPresent("destination", exit);
    setPointIfPresent("to", exit);
    if (Array.isArray(guard.path)) {
      const first = guard.path[0];
      guard.path = [pointLike(first, start), pointLike(first, exit)];
    }

    // A one-aircraft guard must fail when that aircraft is lost. Most versions
    // express this relationally (all lost), but adapt the common explicit forms
    // if they exist on the live template.
    setIfPresent("minSurvivors", 1);
    setIfPresent("requiredSurvivors", 1);
    setIfPresent("maxLosses", 0);
    setIfPresent("allowedLosses", 0);
    setIfPresent("failAtLosses", 1);

    setIfPresent("hudLabel", "PATROL");
    setIfPresent("hitRadio", "PATROL 1被弾！ 偵察機を守れ！");
    setIfPresent("lostRadio", "PATROL 1、信号消失。偵察航路を維持できない。");
    setIfPresent("failRadio", "PATROL 1撃墜。強行偵察は中止する。");
    setIfPresent("safeRadio", "PATROL 1、嵐を抜けた。偵察航路を確保。");
    setIfPresent("asmRadio", "敵機がPATROL 1へ発射！ 母機を落とせ！");
    return Object.freeze(guard);
  }

  const r07Deployment = {};
  Object.defineProperty(r07Deployment, "guard", {
    enumerable: true,
    configurable: false,
    get() {
      const template = liveAirGuardTemplate();
      if (!template) {
        throw new Error("[support-aircraft] m-escort guard template is unavailable");
      }
      return patrolGuardFrom(template);
    }
  });
  ctx.deployFriendlies("r07", r07Deployment);

  // Only the designated interceptors attack PATROL 1. The seven non-target CAP
  // aircraft retain their authored player-focused behaviour, preserving the
  // escort mission's priority choice: stop the hunters while surviving the screen.
  extendMission("r07", (mission) => ({
    ...mission,
    sequence: mission.sequence.map((wave) => (
      wave.tgt !== false && !wave.concurrent
        ? { ...wave, hunt: "air" }
        : wave
    ))
  }));
